...
class AdvancedScoreCalculator {
  async calculateComprehensiveScore(
    ...
    // 2. Apply quality multipliers
    const qualityScore = await this.applyQualityFactors(
      baseScore,
      participation,
    );

    // 3. Apply timing bonuses
    const timingScore = this.applyTimingFactors(
      qualityScore,
      participation,
      context,
    );
    ...
    const socialScore = await this.applySocialFactors(
      timingScore,
      participation,
    );
  ...
  private async applySocialFactors(
    ...
    const socialMultiplier =
      1 + (networkBonus + viralityBonus + resonanceBonus) / 100;
...
const query = `       WITH user_scores AS (
        SELECT 
          user_id,
          SUM(CASE 
...
// Create base season
const seasonData = await tx.query(`         INSERT INTO seasons (theme, start_date, end_date, config)
        VALUES ($1, $2, $3, $4)
        RETURNING *
