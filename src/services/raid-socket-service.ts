...
export class RaidSocketService extends EventEmitter {
  ...
  private setupSocketHandlers() {
    ...
    this.io.on("connection", (socket) => {
      ...
      socket.on(
        "register",
        async (data: { userId: string; username: string; role?: string }) => {
      ...
      socket.on(
        "raid_action",
        async (data: { raidId: string; action: string; metadata?: any }) => {
          const client = this.connectedClients.get(socket.id);
          if (!client) return;

          // Process the action
          const result: any = await this.processRaidAction(
            data.raidId,
            client.userId!,
            data.action,
            data.metadata,
  ...
  private async processRaidAction(
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.SUPABASE_URL}/functions/v1/raid-processor`,
        {
          method: "POST",
          headers: {
  ...
  private async getRaidStats(raidId: string) {
    const { data: stats } = await this.supabase.rpc("calculate_raid_stats", {
      raid_uuid: raidId,
    });
