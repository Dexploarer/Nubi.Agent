...
export class SocketIOEventsService extends Service {
  ...
  capabilityDescription =
    "Real-time events for NUBI features via ElizaOS event system";
  ...
  async initialize(): Promise<void> {
    ...
    logger.info(
      "[SOCKET_EVENTS] Service initialized using ElizaOS event patterns",
    );
  }
  ...
  private registerBroadcastEvents() {
    ...
    for (const eventName of broadcastEvents) {
      ...
      logger.debug(
        `[SOCKET_EVENTS] Registered broadcast handler for: ${eventName}`,
      );
    }
  }
  ...
  private createBroadcastHandler(eventName: string) {
    return async (data: any) => {
      try {
        ...
        logger.error(
          `[SOCKET_EVENTS] Failed to broadcast ${eventName}:`,
          error,
        );
      }
    };
