  ...
  events: {
    MESSAGE_RECEIVED: [
      ...
      async (payload: any) => {
        ...
        const result = await securityFilter.processMessage(userId, message.content.text);
        
        if (!result.allowed) {
          logger.warn(`[SECURITY] Blocked message from ${userId}: ${result.violationType}`);
          
          if (callback) {
            await callback({
              text: result.response || "Your message was blocked by security policy.",
