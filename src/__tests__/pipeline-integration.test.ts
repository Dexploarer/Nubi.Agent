...
import { SocketIOServerService, SocketMessage } from "../services/socket-io-server";
...
describe("Two-Layer Pipeline Integration", () => {
  ...
  describe("System Prompt Routing", () => {
    ...
    it("should inject appropriate system prompts based on classification", async () => {
      ...
      const classification = await messageRouter.classifyMessage(testMessage.message);
      expect(classification.selectedPrompt).toBe("raid-coordinator");
      
      // The system prompt should be injected into message metadata
      const promptContent = messageRouter.getSystemPrompt(classification.selectedPrompt);
  ...
  describe("Error Handling and Resilience", () => {
    it("should gracefully handle ClickHouse unavailability", async () => {
      ...
      mockPipelineAnalytics.logPipelineEvent.mockRejectedValueOnce(new Error("ClickHouse unavailable"));
