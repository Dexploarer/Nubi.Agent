...
describe("DatabasePoolerManager", () => {
  ...
  describe("initialization", () => {
    ...
    it("should have correct service type", () => {
      expect((poolerManager.constructor as any).serviceType).toBe("database-pooler-manager");
    });

    it("should have default connection strings", () => {
      expect(poolerManager.poolerConfig.transactionPooler.connectionString).toContain("6543");
      expect(poolerManager.poolerConfig.sessionPooler.connectionString).toContain("5432");
    });

    it("should have sensible pool configurations", () => {
      expect(poolerManager.poolerConfig.transactionPooler.maxConnections).toBe(20);
      expect(poolerManager.poolerConfig.sessionPooler.maxConnections).toBe(5);
      expect(poolerManager.poolerConfig.transactionPooler.idleTimeoutMillis).toBeLessThan(
  ...
  describe("environment variable handling", () => {
    it("should use environment variables when available", () => {
      ...
      const originalTransactionUrl = process.env.SUPABASE_TRANSACTION_POOLER_URL;
      const originalSessionUrl = process.env.SUPABASE_SESSION_POOLER_URL;
      const originalTransactionMax = process.env.TRANSACTION_POOL_MAX_CONNECTIONS;

      // Set test values
      process.env.SUPABASE_TRANSACTION_POOLER_URL = "postgresql://test:test@localhost:6543/test";
      process.env.SUPABASE_SESSION_POOLER_URL = "postgresql://test:test@localhost:5432/test";
      process.env.TRANSACTION_POOL_MAX_CONNECTIONS = "10";

      const testPooler = new DatabasePoolerManager();
      
      expect(testPooler.poolerConfig.transactionPooler.connectionString).toBe("postgresql://test:test@localhost:6543/test");
      expect(testPooler.poolerConfig.sessionPooler.connectionString).toBe("postgresql://test:test@localhost:5432/test");
  ...
  describe("error handling", () => {
    it("should handle query errors gracefully", async () => {
      ...
      await expect(poolerManager.query("SELECT 1")).rejects.toThrow("Database pooler manager not initialized");
    });

    it("should handle transaction errors gracefully", async () => {
      // Test with uninitialized pooler
      await expect(poolerManager.transaction([{ sql: "SELECT 1" }])).rejects.toThrow("Database pooler manager not initialized");
    });
  });
