...
describe("Plugin Providers", () => {
    ...
    describe("Context Provider", () => {
        ...
        test("should provide context information", async () => {
            ...
            mockRuntime.memoryManager.getMemories = mock(() => 
                Promise.resolve([
                    createMockMessage({ content: { text: "Previous message 1", action: null } }),
                    createMockMessage({ content: { text: "Previous message 2", action: null } }),
                ])
            );
    ...
    describe("API Provider", () => {
        ...
        beforeEach(() => {
            ...
            apiProvider = plugin.providers?.find(
                (provider) => provider.name.includes("api") || provider.name.includes("API")
            );
        });
    ...
    describe("Provider Caching", () => {
        test("providers should utilize cache when available", async () => {
            ...
            const getCalled = (mockRuntime.cacheManager.get as any).mock.calls.length > 0;
            const setCalled = (mockRuntime.cacheManager.set as any).mock.calls.length > 0;
