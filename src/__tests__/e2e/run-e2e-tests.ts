...
class E2ETestRunner {
  ...
  private async runTestSuite(suite: {
    ...
    try {
      ...
      if (result.failed === 0) {
        spinner.succeed(
          chalk.green(`✓ ${suite.name}: ${result.passed} tests passed`),
        );
      } else {
        spinner.fail(
          chalk.red(`✗ ${suite.name}: ${result.failed} tests failed`),
        );
      }
  ...
  private printSummary() {
    ...
    console.log(
      chalk.gray("│ Suite                   │ Passed  │ Failed  │ Duration │"),
    );
    console.log(
      chalk.gray("├─────────────────────────┼─────────┼─────────┼──────────┤"),
    );
    ...
    if (totalFailed === 0) {
      ...
      console.log(
        chalk.gray(
          "Your NUBI/XMCP integration is working correctly with ElizaOS",
        ),
      );
    } else {
      console.log(chalk.red.bold("❌ Some tests failed"));
      console.log(
        chalk.gray("Please review the failures above and fix the issues"),
      );
    }
