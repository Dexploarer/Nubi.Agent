...
export class MockRuntime implements Partial<IAgentRuntime> {
  ...
  updateRecentMessageState = mock(
    async (_state: State): Promise<State> => ({
  ...
  composeState = mock(
    async (_message: Memory, _additionalContext?: any): Promise<State> => ({
      agentId: this.agentId,
      roomId: "test-room" as UUID,
      userId: "test-user" as UUID,
      bio: this.character.bio.join("\n"),
  ...
  completion = mock(
    async (_options: GenerationOptions): Promise<Content> => ({
...
export function createMockRoom(overrides?: Partial<Room>): Room {
  return {
    ...
    // Avoid importing RoomType from @elizaos/core; use a direct literal
    type: "DIRECT" as any,
...
export { describe, expect, test, beforeEach, afterEach, mock, spyOn };
