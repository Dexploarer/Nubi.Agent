...
export class HealthCheckService {
    ...
    async getAuthStatus() {
        ...
        let username = process.env.TWITTER_USERNAME || null;
        let cookiesValid = false;
        ...
        try {
            ...
            if (scraper) {
                ...
                cookiesValid = cookies.some((c) => (c.key === 'auth_token' || c.key === 'ct0') && c.value);
            }
            const status = await authManager.getAuthStatus();
            if (status.hasSavedCookies) {
                authMethod = 'cookies';
    ...
    determineHealthStatus(isAuthenticated, cookiesValid) {
        if (!isAuthenticated) {
            return 'unhealthy';
        }
        ...
        if (!cookiesValid && this.lastSuccessfulAuth) {
            const hoursSinceAuth = (Date.now() - this.lastSuccessfulAuth.getTime()) / (1000 * 60 * 60);
...
export async function performHealthCheck(authConfig) {
