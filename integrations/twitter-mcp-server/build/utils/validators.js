...
export function validateInput(schema, data) {
    try {
        ...
        throw new TwitterMcpError(`Unexpected validation error: ${error.message}`, 'validation_error', 400);
    }
}
/**
 * Validate that media data is properly formatted
 */
export function validateMediaData(mediaData) {
    // Check number of media items
    if (mediaData.length > 4) {
        throw new TwitterMcpError('Maximum of 4 media items allowed per tweet', 'media_validation_error', 400);
    }
    ...
    if (videoCount > 0 && mediaData.length > videoCount) {
        throw new TwitterMcpError('Cannot mix videos and images in the same tweet', 'media_validation_error', 400);
    }
    // Validate each media item
    for (const item of mediaData) {
        // Check media type
        if (!isValidMediaType(item.mediaType)) {
            throw new TwitterMcpError(`Unsupported media type: ${item.mediaType}`, 'media_validation_error', 400);
        }
        ...
        try {
            ...
            if (buffer.length > maxSize) {
                throw new TwitterMcpError(`Media file too large (max ${maxSize / (1024 * 1024)}MB)`, 'media_validation_error', 400);
            }
        }
        catch (error) {
            throw new TwitterMcpError(`Invalid base64 data for media: ${error instanceof Error ? error.message : String(error)}`, 'media_validation_error', 400);
        }
    }
}
/**
...
export function validatePollOptions(options) {
    if (options.length < 2 || options.length > 4) {
        throw new TwitterMcpError('Polls must have between 2 and 4 options', 'poll_validation_error', 400);
    }
    ...
    for (const option of options) {
        if (option.label.length > 25) {
            throw new TwitterMcpError('Poll option labels cannot exceed 25 characters', 'poll_validation_error', 400);
        }
    }
}
